...
(function() {
  $(function() {
    ...
    return readCSVFile = function() {
      var csvContents, reader;
      csvContents = $('#CSVfile')[0].files[0];
      reader = new FileReader();
      reader.onload = function(event) {
        return reader.result.split('\n').forEach(function(line) {
          var $tr, requestParams;
          $tr = $('<tr>');
          $tr.append($('<td>' + line + '</td>'));
          requestParams = {
            url_string: line,
            search_term: $('#search-term').val(),
            include_dynamic_tags: $('#include_dynamic_tags').checked
          };
          return $.getJSON('/ping_url', requestParams).done(function(result) {
            result.forEach(function(item) {
              return $tr.append($('<td>' + item + '</td>'));
            });
            return $('#test-results tbody').append($tr);
          });
        });
      };
      return reader.readAsText(csvContents);
    };
  });
